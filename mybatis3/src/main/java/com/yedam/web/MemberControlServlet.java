...
@WebServlet("/member.action") // url 호출
public class MemberControlServlet extends HttpServlet {// HttpServlet 상속

	public MemberControlServlet() { // 최초요청일 경우에만 객체를 생성함
		System.out.println("MemberControl 객체 생성");
	}

	@Override // init ctrl spacebar 두번째꺼
	public void init(ServletConfig config) throws ServletException {
		System.out.println("최초요청이면 init 실행");
	}

	@Override // service ctrl spacebar 첫번째꺼
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		resp.setContentType("text/html;charset=utf-8");

		System.out.println("서블릿을 요청할때마다 실행");// 최초 실행후 다시실행을 해도 서블릿만 실행
		// get:조회화면 , post:삭제처리.
		...
		if (req.getMethod().equals("GET")) {
			...
			Member member = dao.selectMember(id); // 해당되는 id가 없으면 null
	...
	@Override
	public void destroy() { // dest ctrl spacebar 첫번째꺼
